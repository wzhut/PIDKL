...
class PIGPR:
    ...
    def __init__(self, X_u, u, X_f, layers, lb, ub):
        ...
        #self.tf_log_lengthscale = tf.constant(0.0, dtype=tf.float32)
        ...
        for i in range(len(self.x_f_tf)):
            ...
            f_pred = self.pde_val(self.x_f_tf[i])
    ...
    def pde_val(self, t):
        m,v = self.pred(t)
        u = m + tf.sqrt(v)*tf.random.normal(tf.shape(v))
        u_t = tf.gradients(u, t)[0]
        f = u_t + tf.exp(self.tf_log_D)*u - self.tf_B
        return f
    ...
    def train(self, X_test, y_test):
        ...
        for i in range(30000):
            ...
            if (i<13200 and i%100==0) or (i>=13200 and i%1==0):
                y_pred = model.test(X_test)
                y_mean = y_pred[0]
                y_var = y_pred[1]
                #error = np.linalg.norm(y_mean - y_test, 2)/np.linalg.norm(y_test, 2)
                error = np.sqrt(np.mean((y_mean - y_test)**2))
                print('epoch %d, Error u: %e' % (i,error))                     
                with open('res.txt', 'a') as f:
                    f.write('epoch %d, rmse = %g\n'%(i, error))
