...
class PIDKL:
    def __init__(self, cfg):
        ...
        self.B = torch.tensor([1.], dtype=torch.float64, device=self.device, requires_grad=True)
        ...
        self.params = [self.log_ls_f, self.log_ls_g, self.log_tau, self.B, self.log_D] + list(self.nn.parameters())
    ...
    def get_nELBO(self):
        ...
        if torch.isinf(loss) or torch.isnan(loss):
            print(L)
            print(L_reg)
            print(torch.exp(self.log_ls_f))
            print(torch.logdet(S))            
            input()
        return loss
    ...
    def pde(self, X_g):
        ...
        u = u_mean + u_std * eta
        u_t = torch.autograd.grad(u.sum(), X_g, create_graph=True)[0]
        f = u_t + torch.exp(self.log_D) * u - self.B
        #f = u_t + u - 1
        return f
...
if __name__ == '__main__':
    ...
    cfg = {
        ...
        'test_X': test_X,
        'test_y': test_y,
        # 'co_X': np.linspace(0, 1, 100).reshape((-1, 1)),
        'gamma': 0.1
    }
